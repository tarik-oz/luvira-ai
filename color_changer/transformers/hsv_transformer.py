    ...
    def apply_hsv_transformations(
        ...
        try:
            ...
            if light_cfg and light_cfg.get('enabled', False):
                ...
                base_v = image_hsv[:, :, 2]
                hair_pixels = (base_v * mask_normalized)[mask_normalized > 0.1]
        ...
        if tone_label is not None:
            try:
                ...
                result_hsv[:, :, 0] = np.where(smooth_mask > 0.1, h, result_hsv[:, :, 0])
                ...
                result_hsv[:, :, 1] = np.where(smooth_mask > 0.1, np.clip(s, 0, 255), result_hsv[:, :, 1])

                # Value: gentle nudge toward target to keep separability
                v = result_hsv[:, :, 2]
                v = v + (target_hsv[2] - v) * (0.08 * alpha)
                result_hsv[:, :, 2] = np.where(smooth_mask > 0.1, np.clip(v, 0, 255), result_hsv[:, :, 2])
            except Exception:
                pass
