...
const imageModules = import.meta.glob('../assets/hair_pattern_colors/*/*.png', {
  import: 'default',
  eager: true,
...
export const getBasePreview = (colorName: string): string => {
  ...
  const path = `../assets/hair_pattern_colors/${slug}/${slug}_base.png`
...
export const getTonePreview = (colorName: string, toneName: string): string => {
  ...
  const path = `../assets/hair_pattern_colors/${color}/${color}_${tone}.png`
  return resolvePath(path) || ''
}
