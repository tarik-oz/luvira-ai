...
def save_timestamps(processed_dir, image_ts, mask_ts):
    ...
    print(f"Timestamps file saved: {ts_path}")
...
def load_timestamps(processed_dir):
    ...
    with open(ts_path, "r") as f:
        print(f"Timestamps file loaded: {ts_path}")
        return json.load(f)
...
def needs_processing(images_dir, masks_dir, processed_dir):
    ...
    if saved_ts is None:
        print("No timestamps found. Data will be processed for the first time.")
        return True
    if image_ts != saved_ts["images"] or mask_ts != saved_ts["masks"]:
        print("Change detected in images or masks. Data will be reprocessed.")
        return True
    print("Data is up-to-date. Existing processed npy files will be used.")
    return False 