...
class PredictionService:
    ...
    async def predict_mask_file(self, file) -> bytes:
        """
        ...
        try:
            ...
            image = await self._process_uploaded_image(file)
            
            # Create temporary file for prediction
            temp_input_path = self._create_temp_image_file(image)
            
            # Make prediction
            predicted_mask = self._make_prediction(temp_input_path)
    ...
    async def _process_uploaded_image(self, file) -> np.ndarray:
    ...
    def _make_prediction(self, temp_input_path: str) -> np.ndarray:
